...
export default function SmokeBackground() {
	...
	useEffect(() => {
		if (process.env.NODE_ENV === "production") return; // test local d'abord
		const canvas = canvasRef.current;
		if (!canvas) return;

		// Adapter le JS du CodePen pour un canvas en arrière-plan et des events sur window
		...
		// Activer le blending pour conserver le background app en transparence
		gl.enable(gl.BLEND);
		gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);

		// Seed initial (fumée visible sans interaction)
		splatStack.push(20);
		// Assurer une composition transparente avec le fond
		gl.enable(gl.BLEND);
		gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
