...
export default function SmokePenPage() {
	useEffect(() => {
		// ----- CodePen JS (adapté) -----
		"use strict";
		...
		canvas.addEventListener("mousemove", function (e: MouseEvent) {
			...
			pointers[0].dx = (e.offsetX - p0.x) * 10.0;
			pointers[0].dy = (e.offsetY - p0.y) * 10.0;
			pointers[0].x = e.offsetX as any;
			pointers[0].y = e.offsetY as any;
		});
		...
		function m(t: string) {
			...
			const arr = n.innerHTML.replace("&amp;", "&").split("");
			let a = "";
			for (let o = 0; o < arr.length; o++) {
				const e = arr[o].replace("&", "&amp");
				a += e.trim()
